@extends('layouts.app', ['title' => 'Edit Medication Report'])
@section('title', 'Edit Medication Report')

@section('content')
@php
    $prevTypes   = old('med_type',   array_map('trim', explode(',', $medication->incident_type   ?? '')));
    $prevCauses  = old('med_cause',  array_map('trim', explode(',', $medication->cause_factor    ?? '')));
    $prevActions = old('med_action', array_map('trim', explode(',', $medication->immediate_action ?? '')));
    $prevFollow  = old('med_follow', array_map('trim', explode(',', $medication->follow_up_action ?? '')));
@endphp

<div class="mx-auto max-w-full px-4 py-6">

    <form id="medication-form"
          action="{{ route('forms.medication.update', $medication->id) }}"
          method="POST"
          novalidate
        ...
        <div class="border-b border-slate-200 bg-slate-50 px-6 py-5 dark:border-slate-800 dark:bg-slate-900 flex items-center justify-between">
            <div>
                <h1 class="text-xl font-black text-slate-900 dark:text-white">Edit Medication Report</h1>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Record #{{ $medication->id }}</p>
            </div>
        ...
        <div class="space-y-8 p-6">
            ...
            <section>
                ...
                <div class="grid gap-4 md:grid-cols-12">
                    <div class="md:col-span-6">
                        ...
                        <input type="text" name="pr_name" value="{{ old('pr_name', $medication->pr_name) }}" class="form-input" data-required>
                    </div>
                    <div class="md:col-span-6">
                        <label class="form-label">Position <span class="text-red-500">*</span></label>
                        <input type="text" name="pr_position" value="{{ old('pr_position', $medication->pr_position) }}" class="form-input" data-required>
                    </div>
                    <div class="md:col-span-4">
                        <label class="form-label">Date incident/error occurred <span class="text-red-500">*</span></label>
                        <input type="date" name="pr_date_occured" value="{{ old('pr_date_occured', $medication->pr_date_occured) }}" class="form-input" data-required>
                    </div>
                    <div class="md:col-span-4">
                        <label class="form-label">Time of incident/error <span class="text-red-500">*</span></label>
                        <input type="time" name="pr_time_occured" value="{{ old('pr_time_occured', $medication->pr_time_occured) }}" class="form-input" data-required>
                    </div>
                    <div class="md:col-span-4">
                        <label class="form-label">Date incident/error reported <span class="text-red-500">*</span></label>
                        <input type="date" name="pr_date_reported" value="{{ old('pr_date_reported', $medication->pr_date_reported) }}" class="form-input" data-required>
                    </div>
                    <div class="md:col-span-12">
                        <label class="form-label">Incident/Error reported to <span class="text-red-500">*</span></label>
                        <input type="text" name="pr_incident_reported_to" value="{{ old('pr_incident_reported_to', $medication->pr_incident_reported_to) }}" class="form-input" data-required>
                    </div>
                </div>
            </section>
            ...
            <section class="border-t border-slate-200 pt-8 dark:border-slate-800">
                ...
                <div class="grid gap-4 md:grid-cols-12">
                    <div class="md:col-span-6">
                        ...
                        <input type="text" name="cd_name" value="{{ old('cd_name', $medication->cd_name) }}" class="form-input" data-required>
                    </div>
                    <div class="md:col-span-6">
                    ...
                    <div class="md:col-span-12">
                        ...
                        <input type="text" name="cd_responsible" value="{{ old('cd_responsible', $medication->cd_responsible) }}" class="form-input">
                    </div>
                </div>
            </section>
            ...
            <section class="border-t border-slate-200 pt-8 dark:border-slate-800">
                ...
                <div class="mt-2 grid gap-2 sm:grid-cols-2 lg:grid-cols-3" id="incident-type-group">
                    @foreach(['Wrong Client/Participant','Wrong Date','Wrong Time','Wrong Type of Medication','Wrong Route','Wrong Dose','Incorrect Documentation','Pharmacy Error','Client refusal','Other'] as $type)
                        <label class="check-card">
                            <input type="checkbox" name="med_type[]" value="{{ $type }}"
                                   {{ in_array($type, $prevTypes) ? 'checked' : '' }}
                                   class="peer sr-only">
                ...
                <div class="mt-5">
                    ...
                    <textarea name="incident_background" rows="4" class="form-input resize-none" data-required>{{ old('incident_background', $medication->incident_background) }}</textarea>
                </div>

                <div class="mt-5">
                    <label class="form-label">Details of incident/error <span class="text-red-500">*</span></label>
                    <textarea name="incident_details" rows="4" class="form-input resize-none" data-required>{{ old('incident_details', $medication->incident_details) }}</textarea>
                </div>

                <div class="mt-5">
                    <label class="form-label">Cause(s) or contributing factor(s)</label>
                    <div class="mt-2 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                        @foreach(['Missing Information','Missing Labels','Storage','Delivery Device issues','Environmental','Training Deficit','Other'] as $cause)
                            <label class="check-card">
                                <input type="checkbox" name="med_cause[]" value="{{ $cause }}"
                                       {{ in_array($cause, $prevCauses) ? 'checked' : '' }}
                                       class="peer sr-only">
                                <span class="check-box"></span>
                                <span>{{ $cause }}</span>
                            </label>
                ...
                <div class="mt-5">
                    ...
                    <div class="mt-2 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                        @foreach(['Notified to on call','Called nurse On call','Notified to Line Manager','Notified Client/Participant','Notified Pharmacy','Notified General Practitioner','Telephoned Ambulance','Other'] as $action)
                            <label class="check-card">
                                <input type="checkbox" name="med_action[]" value="{{ $action }}"
                                       {{ in_array($action, $prevActions) ? 'checked' : '' }}
                                       class="peer sr-only">
                                <span class="check-box"></span>
                                <span>{{ $action }}</span>
                            </label>
            ...
            <section class="border-t border-slate-200 pt-8 dark:border-slate-800">
                ...
                <div class="mt-2 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach(['Resolved with Pharmacy','Resolved with General Practitioner','Further training provided','Environmental factors resolved','Other'] as $follow)
                        <label class="check-card">
                            <input type="checkbox" name="med_follow[]" value="{{ $follow }}"
                                   {{ in_array($follow, $prevFollow) ? 'checked' : '' }}
                                   class="peer sr-only">
                            <span class="check-box"></span>
                            <span>{{ $follow }}</span>
                        </label>
                ...
                <div class="mt-5">
                    ...
                    <textarea name="action_explaination" rows="5" class="form-input resize-none">{{ old('action_explaination', $medication->action_explaination) }}</textarea>
                </div>

                <div class="mt-5 grid gap-4 md:grid-cols-12">
                    <div class="md:col-span-4">
                        <label class="form-label">Action taken by</label>
                        <input type="text" name="action_taken_by" value="{{ old('action_taken_by', $medication->action_taken_by) }}" class="form-input">
                    </div>
                    <div class="md:col-span-4">
                        <label class="form-label">Date completed</label>
                        <input type="date" name="date_completed" value="{{ old('date_completed', $medication->date_completed) }}" class="form-input">
                    </div>
                    <div class="md:col-span-4">

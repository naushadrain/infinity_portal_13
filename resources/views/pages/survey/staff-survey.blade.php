...
@section('content')
    ...
    <div
        ...
        <form method="GET" action="{{ route(request()->route()->getName()) }}"
            ...
            <div
                class="flex items-center gap-2 flex-1 min-w-[220px] max-w-md bg-slate-50 dark:bg-ink-800 rounded-lg px-3 py-2">
            ...
            <button type="submit" class="px-3 py-2 rounded-lg bg-brand-600 text-white text-sm flex items-center gap-2">
            ...
            <table class="w-full">
                <thead class="bg-slate-50 dark:bg-ink-800">
                    <tr>
                        ...
                        <th
                            class="text-left font-medium text-xs text-slate-500 dark:text-ink-400 uppercase tracking-wider px-4 py-3">
                            Satisfied With Career Growth</th>
                        <th
                            class="text-left font-medium text-xs text-slate-500 dark:text-ink-400 uppercase tracking-wider px-4 py-3">
                            Job Responsibilities Match Your Strengths?</th>
                        <th
                            class="text-left font-medium text-xs text-slate-500 dark:text-ink-400 uppercase tracking-wider px-4 py-3">
                            Work Life Balance</th>
                        <th
                            class="text-left font-medium text-xs text-slate-500 dark:text-ink-400 uppercase tracking-wider px-4 py-3">
                            Comfortable Feel Voicing Concerns</th>
                        <th
                            class="text-left font-medium text-xs text-slate-500 dark:text-ink-400 uppercase tracking-wider px-4 py-3">
                            Feel Stressed Work</th>
                        <th
                            class="text-left font-medium text-xs text-slate-500 dark:text-ink-400 uppercase tracking-wider px-4 py-3">
                            Meaningful Feel Your Work</th>
                        <th
                            class="text-left font-medium text-xs text-slate-500 dark:text-ink-400 uppercase tracking-wider px-4 py-3">
                            Rating</th>
                    </tr>
        ...
        <div
            ...
            <div>
                Showing {{ $staffSurveys->firstItem() ?? 0 }}–{{ $staffSurveys->lastItem() ?? 0 }} of
                {{ $staffSurveys->total() }}
            </div>
